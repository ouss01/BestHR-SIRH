...
from .models import (
    ...
    Affectation, EmploymentHistory, Task, Notification
)
from .serializers import (
    ...
    EmploymentHistorySerializer, TaskSerializer
)
...
def send_welcome_email(subject, context, from_email, recipient_email):
    """
    Sends a welcome email using an HTML template.

    Args:
        subject (str): The subject of the email.
        context (dict): The context for rendering the email template.
    ...
    html_message = render_to_string("template/email.html", context)

    # Send the email with only the HTML content
    send_mail(
        subject=subject,
        message="",  # Plain text version is omitted
        from_email=from_email,
        recipient_list=[recipient_email],
        html_message=html_message,  # Set the HTML message
    )
...
class EmployeeRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        # You can customize how the update is performed here if needed
        serializer.save()
...
from .serializers import OnboardingSerializer, EtapeOnboardingSerializer, TaskSerializer, NotificationSerializer, \
    ...
    if request.method == 'POST':
        try:
            ...
            send_welcome_email(
                ...
                context={'username': username},  # Context for the HTML template
                from_email='oussematrimech@gmail.com',  # Sender's email
